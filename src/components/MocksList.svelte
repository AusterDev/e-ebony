...
>
  {#each mocks.slice((currentPage - 1) * pageSize, currentPage * pageSize) as mock (mock.id)}
    <div
    ...
    >
      <div>
        <div class="flex items-center gap-3 mb-3">
          ...
          <div>
            <h2 class="text-lg font-bold text-slate-900">{mock.subject}</h2>
            <p class="text-sm text-slate-500">ID: {mock.id}</p>
          </div>
        </div>

        <div class="text-slate-700 text-sm mb-4 line-clamp-3">
          {@html mock.instructions || "No instructions provided."}
        </div>
      </div>

      <div
        class="flex justify-between items-center text-sm text-slate-600 border-t border-slate-100 pt-3 mt-3"
      >
        <div>
          <span class="font-semibold">{mock.totalTimeMins}</span> mins
        </div>
        <a
          href={`/mock/${mock.id}`}
